...
if __name__ == '__main__':
    with open('./addresses_for_task.csv', ) as f1, open('./Counties.geojson', ) as f2, open(
            './Townlands.geojson', ) as f3:
        all_addresses = list(csv.DictReader(f1))
        counties_dataset = json.load(f2)['features']
        townlands_dataset = json.load(f3)['features']

        # response = requests.get('https://data.gov.ie/api/3/action/package_show', {'id': 'railway-stations-osi-national-250k-map-of-ireland'})
